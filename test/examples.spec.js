import { combineSelectors, composeSelectors, createSelector, filterState, withOptions, withProps, withState } from '../src'
  ...
  describe('examples', () => {
    ...
    describe('withOptions(creator)', () => {
      it('creates a configurable selector', () => {
        ...
        expect(
          selectAppleById(ownProps.id)(state)
        ).toEqual(
          { id: 1, size: 'big' }
        )
      })
      it('creates a selector configured by props', () => {
        const selectApples = createSelector('department.produce.fruit.apples')
        const selectAppleById = withOptions(props => composeSelectors(
          selectApples,
          apples => apples.find(apple => apple.id === props.id)
        ))

        expect(
          selectAppleById(ownProps)(state)
        ).toEqual(
          { id: 1, size: 'big' }
        )
      })
    ...
    describe('withOptions(creator, attrFilter)', () => {
      it('creates a configurable selector that only receives state', () => {
        ...
        expect(
          selectAppleById(ownProps.id)(state, ownProps)
        ).toEqual(
          { id: 1, size: 'big' }
        )
      })
    ...
    describe('withProps(creator)', () => {
      it('creates a configurable selector that is configured by props', () => {
        ...
        expect(
          selectFruitById(state, ownProps)
        ).toEqual(
          { id: 1, size: 'big' }
        )
      })
    ...
    describe('withProps(creator, ...propsSelectors)', () => {
      it('creates a configurable selector that is configured by selected props', () => {
        const selectFruit = createSelector('department.produce.fruit')
        const selectFruitById = withProps((id, type) => composeSelectors(
          selectFruit,
          type,
    ...
    describe('composeSelectors(...selectors)', () => {
      it('creates a composed selector', () => {
        ...
        expect(
          selectFruitSize(state, ownProps)
        ).toEqual(
          'big'
        )
      })
